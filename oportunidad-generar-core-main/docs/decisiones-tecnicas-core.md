...
| --- | --- | --- | --- | --- |
| D01 | Mantener `oportunidad-generar-core.xaml` como fachada publica estable. | Cerrada | El padre debe invocar un unico submodulo CORE sin conocer la composicion interna. | Los subworkflows viven en `lib`; el padre solo llama a CORE. |
...
| D21 | Mantener comparador de equivalencia CORE como puerta de rendimiento. | Cerrada | Las optimizaciones de Excel pueden cambiar valores, formulas o rojo funcional sin romper validadores parciales. | `test/comparar_outputs_core_equivalencia.ps1` compara PC/AT contra `data/baseline`, normalizando solo el timestamp documentado de `Cost Summary!B18`. |
| D22 | Sustituir loops/celdas individuales por batches COM acotados cuando UiPath genera exceso de eventos. | Cerrada | El profile inicial agotaba 40000 eventos por lecturas amplias y barridos celda a celda. | Project Infor y el marcado final de errores usan mapa/rangos contractuales, cabecera de `Invoke Code`, guardado controlado y liberacion COM. |
| D23 | Conservar limpiezas Excel cuando protegen formato residual o contrato estatico. | Cerrada | No todo `ClearRangeX` es eliminable: algunos rangos preservan ausencia de rojo residual o estan protegidos por validadores. | EP4 retiro solo pares seguros; permanecen las limpiezas justificadas en PC Resources, PC compras meses y compras AT. |
| D24 | Leer capacidad de plantilla por cortes minimos con fallback amplio y cache en memoria. | Cerrada | La plantilla oficial mantiene capacidad 60, pero no debe asumirse como constante fija. | Se leen marcadores/cabeceras minimos, se cae a rangos historicos si hay ambiguedad y la cache se invalida por tipo, ruta, tamano y timestamp. |
| D25 | Diferenciar runtime final de profile Studio. | Cerrada | `UiRobot.exe execute` no genera `.uistat` en esta instalacion local. | La evidencia CLI final mide PC+AT completo y valida equivalencia; `ActivityExecutionLimitReached=False` queda certificable solo con nuevo profile desde Studio. |
| D26 | Cerrar cambios de rendimiento con suite completa y sin Excel residual. | Cerrada | En un proyecto Enterprise el rendimiento no justifica desviaciones funcionales ni procesos huerfanos. | EP6 valida calidad E06, PC 12 meses, AT 48 meses, comparador PC/AT y `Get-Process Excel` sin procesos activos al cierre. |
